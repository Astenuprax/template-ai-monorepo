...
Shared runtime for the [`template-ai-monorepo`](../../README.md) reference template: a typed
...
a small, deterministic, self-demonstrating example of an agent-plus-MCP-tool architecture.
...
The packaged console script runs the agent end-to-end (needs `ANTHROPIC_API_KEY`):
...
|---|---|
| `agent.py` | the pydantic-ai agent, wired to its tool over MCP |
...
Licensed Apache-2.0. See the [workspace README](../../README.md) for architecture and contribution guidance.
